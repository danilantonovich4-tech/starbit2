...
def get_accounts_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="Без спамблока", callback_data="cat_no_spamblock")],
...
def get_pagination_menu(category, sort_type, page):
    ...
    nav_buttons = []
    if page > 0:
        nav_buttons.append(InlineKeyboardButton(text="⬅️", callback_data=f"page_{category}_{sort_type}_{page - 1}"))
...
def get_admin_confirm_menu(order_id):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Подтвердить сделку", callback_data=f"confirm_{order_id}")]
    ])
...
async def select_sort(call: CallbackQuery):
    ...
    if not accounts_db[category]:
        await call.answer("В этой категории пока нет аккаунтов.", show_alert=True)
        return
    await call.message.edit_text("Как отсортировать аккаунты?", reply_markup=get_sort_menu(category))
...
async def buy_stars(call: CallbackQuery, state: FSMContext):
    await call.message.edit_text(f"Количество звезд (От {MIN_STARS} шт. Курс {STAR_PRICE} руб):")
...
async def process_stars_username(message: Message, state: FSMContext):
    ...
    text = f"Оплата {total_price} руб за {amount} звезд.\nКарта: {CARD_NUMBER}\nЖмите кнопку после оплаты."
...
async def check_payment(call: CallbackQuery):
    ...
    if order['type'] == 'stars':
        admin_text += f"Количество: {order['amount']}\nПолучатель: {order['target_user']}"
...
async def admin_confirm_order(call: CallbackQuery):
    ...
    if order["type"] == "account":
        ...
        if not acc_data:
            await call.message.answer("Этот аккаунт уже продан!")
            return
        ...
        await bot.send_message(chat_id=user_id, text=f"Заказ выполнен! Звезды отправлены {order['target_user']}.")
...
async def admin_country_saved(message: Message, state: FSMContext):
    ...
    await message.answer("Данные аккаунта:")
